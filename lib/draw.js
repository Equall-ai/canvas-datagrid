...
export default function (self) {
  ...
  function drawTextDecoration(cell, line) {
    var deco = cell.textDecoration,
      prevFill = self.ctx.fillStyle,
      w,
      y,
      thickness;
    if (!deco || deco === 'none') {
    ...
    // line.width accumulates the width of each word *including* its trailing
    // split character, so it overshoots the last glyph. Measure the trimmed
    // value so the rule stops where the text does.
    w = self.ctx.measureText(line.value.replace(/\s+$/, '')).width;
    if (w <= 0) {
      return;
    }
    // cell.fontHeight is already multiplied by self.scale, so the proportional
    // defaults below are scale-correct. An explicit width is in CSS pixels.
    thickness = Math.max(
      1,
      cell.textDecorationWidth
        ? cell.textDecorationWidth * self.scale
        : cell.fontHeight * 0.07,
    );
    // Offsets are measured from the alphabetic baseline: a line-through sits
    // near the middle of the x-height, an underline just below the baseline.
    y =
      deco === 'underline'
        ? line.y + cell.fontHeight * 0.16
        : line.y - cell.fontHeight * 0.3;
    if (cell.textDecorationColor) {
      self.ctx.fillStyle = cell.textDecorationColor;
    }
    fillRect(line.x, y - thickness / 2, w, thickness);
    self.ctx.fillStyle = prevFill;
  }
  function drawFlagIcon(cell) {
    var ctx = self.ctx,
      prevFill = ctx.fillStyle,
      prevStroke = ctx.strokeStyle,
      prevLineWidth = ctx.lineWidth,
      prevLineCap = ctx.lineCap,
      // cell.fontHeight and cell.paddingRight are already multiplied by
      // self.scale; an explicit size is in CSS pixels.
      size = cell.flagIconSize
        ? cell.flagIconSize * self.scale
        : cell.fontHeight,
      // The glyph geometry below comes from a 24x24 source SVG; u maps one
      // of its units onto the icon box.
      u = size / 24,
      color = cell.flagIconColor || self.style.cellFlagIconColor,
      x,
      y;
    // Skip cramped cells rather than drawing over their text.
    if (cell.width < size * 2.5) {
      return;
    }
    // Raw ctx path calls do not add the canvas offsets the way the
    // fillRect/fillText helpers do, so bake them into the origin.
    x = cell.x + cell.width - cell.paddingRight - size + self.canvasOffsetLeft;
    y = cell.y + (cell.height - size) / 2 + self.canvasOffsetTop;
    ctx.fillStyle = color;
    ctx.strokeStyle = color;
    // banner: wavy top edge left-to-right, right edge down, wavy bottom
    // edge back, closed against the pole — filled
    ctx.beginPath();
    ctx.moveTo(x + 4 * u, y + 2.997 * u);
    ctx.bezierCurveTo(
      x + 6.8 * u, y + 1.003 * u,
      x + 10.5 * u, y + 2.27 * u,
      x + 12 * u, y + 3.1 * u,
    ...
    ctx.bezierCurveTo(
      x + 15.6 * u, y + 4.695 * u,
      x + 18.833 * u, y + 3.661 * u,
      x + 20 * u, y + 2.996 * u,
    );
    ctx.lineTo(x + 20 * u, y + 15.023 * u);
    ctx.bezierCurveTo(
      x + 18.833 * u, y + 15.688 * u,
      x + 15.6 * u, y + 16.722 * u,
      x + 12 * u, y + 15.127 * u,
    );
    ctx.bezierCurveTo(
    ...
    ctx.closePath();
    ctx.fill();
    // pole: round-capped stroke, as in the source SVG
    ctx.beginPath();
    ctx.lineWidth = Math.max(1, 1.5 * u);
    ctx.lineCap = 'round';
    ctx.moveTo(x + 4 * u, y + 2.997 * u);
    ctx.lineTo(x + 4 * u, y + 21.933 * u);
    ctx.stroke();
  ...
  self.draw = function (internal) {
    ...
    function drawCell(rowData, rowOrderIndex, rowIndex) {
      ...
      return function drawEach(header, headerIndex, columnOrderIndex) {
        ...
        if (active && cellStyle !== 'cornerCell' && !self.attributes.animatedCellSelection) {
        ...
        if (selected && (!self.attributes.animatedCellSelection || (!active && self.selections && self.selections.length > 1))) {
          self.ctx.fillStyle =
        ...
        if ((self.attributes.showRowNumbers && isRowHeader) || !isRowHeader) {
          if (cell.isGrid && !self.dispatchEvent('beforerendercellgrid', ev)) {
            ...
            if (!self.dispatchEvent('rendertext', ev)) {
              ...
              // Set on the cell by rendertext handlers (e.g. the Svelte
              // wrapper's cellStyle `flag` property).
              if (cell.flagIcon) {
                drawFlagIcon(cell);
              }
        ...
        if (selected && !cell.isHeader && !cell.isRowHeader && self.attributes.animatedCellSelection) {
          selectionCellCount++;
          if (!selectionRect) {
            selectionRect = { x: cell.x, y: cell.y, x2: cell.x + cell.width, y2: cell.y + cell.height };
          } else {
            if (cell.x < selectionRect.x) selectionRect.x = cell.x;
            if (cell.y < selectionRect.y) selectionRect.y = cell.y;
            if (cell.x + cell.width > selectionRect.x2) selectionRect.x2 = cell.x + cell.width;
            if (cell.y + cell.height > selectionRect.y2) selectionRect.y2 = cell.y + cell.height;
          }
        }
    ...
    function drawSelectionHandles() {
      if (self.selections && self.selections.length > 1) return;
      if (
    ...
    function drawActiveCell() {
      if (!aCell) {
        if (self.updateSelectionOverlay) {
          self.updateSelectionOverlay(null);
        }
        return;
      }
      if (self.attributes.animatedCellSelection && self.updateSelectionOverlay) {
        // Multi-select (meta+click): overlay on active cell only, backgrounds on others
        // Single/drag select: overlay wraps the entire selection rect
        var isMultiSelect = self.selections && self.selections.length > 1;
        var overlayTarget = (!isMultiSelect && selectionRect && selectionCellCount > 1)
          ? { x: selectionRect.x, y: selectionRect.y, width: selectionRect.x2 - selectionRect.x, height: selectionRect.y2 - selectionRect.y }
          : aCell;
        self.updateSelectionOverlay(overlayTarget);
        return;
      }
      self.ctx.save();
      var cl =
    ...
    drawResizeMarkers();
    drawBorder();
    if (!self.attributes.animatedCellSelection) {

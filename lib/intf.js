...
export default function (self, ctor) {
  ...
  self.init = function () {
    ...
    self.intf.addSelection = self.addSelection;
    self.intf.removeSelection = self.removeSelection;
    self.intf.setSelectionList = self.setSelectionList;
  ...
  // Right edge of the pinned (frozen) column block as laid out by the last
  // draw pass, in canvas pixels — the same space as visibleCells x values.
  // Divide by `scale` for CSS pixels.  0 when no frozen columns.  Exposed so
  // DOM overlays (e.g. the Svelte wrapper) can clip against the engine's own
  // boundary instead of re-deriving it from visibleCells.
  Object.defineProperty(self.intf, 'frozenColumnPixel', {
    get: function () {
      return self.frozenColumn > 0 ? self.lastFrozenColumnPixel || 0 : 0;
    },
  });

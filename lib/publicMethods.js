...
export default function (self) {
  ...
  self.gotoCell = function (x, y, offsetX, offsetY) {
    ...
    if (x !== undefined && y !== undefined) {
      ...
      // Minimal scroll: only move enough to reveal the target column
      var currentScrollLeft = self.scrollBox.scrollLeft;
      var columnRightEdge = self.scrollCache.x[x];
      var columnLeftEdge = x > 0 ? self.scrollCache.x[x - 1] : 0;
      var rowHeaderWidth = self.getRowHeaderCellWidth();
      var frozenWidth =
        self.frozenColumn > 0
          ? self.scrollCache.x[self.frozenColumn - 1]
          : 0;
      var visibleDataWidth = self.scrollBox.width - rowHeaderWidth - frozenWidth;

      if (columnRightEdge > currentScrollLeft + visibleDataWidth + frozenWidth) {
        // Column is off-screen to the right: align its right edge with the visible right edge
        self.scrollBox.scrollLeft =
          columnRightEdge - visibleDataWidth - frozenWidth;
      } else if (columnLeftEdge < currentScrollLeft + frozenWidth) {
        // Column is off-screen to the left: align its left edge with the visible left edge
        self.scrollBox.scrollLeft = Math.max(0, columnLeftEdge - frozenWidth);
      }
    } else if (y !== undefined) {
  ...
  self.scrollIntoView = function (x, y, offsetX, offsetY) {
    // A non-frozen cell is hidden if its left edge is behind the frozen
    // column boundary, or its top edge is behind the frozen row boundary.
    const frozenLeft = self.lastFrozenColumnPixel || self.getRowHeaderCellWidth();
    const frozenTop = self.lastFrozenRowPixel || self.getColumnHeaderCellHeight();
    const matched = self.visibleCells.filter(function (cell) {
      const isFrozenCol = x !== undefined && x < self.frozenColumn;
      const isFrozenRow = y !== undefined && y < self.frozenRow;
      const minX = isFrozenCol ? 0 : frozenLeft;
      const minY = isFrozenRow ? 0 : frozenTop;
      return (

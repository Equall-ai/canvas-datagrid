...
function Grid(args) {
  ...
  if (self.isChildGrid) {
    self.intf = {};
  } else if (self.isComponent) {
    // `Grid` is the constructor registered with customElements.define below,
    // so it is the correct new.target for every path that reaches here.
    // Referenced directly rather than written as `new.target` because Babel
    // down-levels `new.target` for our browserslist targets into a
    // `this.constructor` lookup that resolves to HTMLElement.  The previous
    // workaround was to hide the expression from Babel inside eval(), which
    // throws under a Content-Security-Policy without 'unsafe-eval' and aborts
    // grid construction part-way through.
    self.intf = Reflect.construct(HTMLElement, [], Grid);
  } else {
